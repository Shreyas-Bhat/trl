...
class RLOOTrainer(Trainer):
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                # sequence_length = first_true_indices(query==processing_class.pad_token_id) - 1
                # llm_output = forward(self.llm_decision_maker, query, processing_class.pad_token_id)
                ...
                _, score, _ = get_reward(
                    self.llm_decision_maker, postprocessed_query_response, processing_class.pad_token_id, context_length, llm_scores=llm_scores, ground_truth=ground_truth_batch, tokenizer=AutoTokenizer.from_pretrained("microsoft/Phi-3-mini-4k-instruct")
                ...
                # )
                # _, score, _ = get_reward(
                #     self.llm_decision_maker, query, processing_class.pad_token_id, context_length, llm_scores=llm_scores, ground_truth=ground_truth_batch, tokenizer=AutoTokenizer.from_pretrained("Qwen/Qwen2-0.5B")
                # )
    ...
    def generate_completions(self, sampling: bool = False):
        ...
        generation_config = GenerationConfig(
            ...
            top_p=0.9,
            pad_token_id=processing_class.pad_token_id,
            suppress_tokens=[processing_class.eos_token_id],
            renormalize_logits=True,
            no_repeat_ngram_size=3,
            repetition_penalty=1.2,
        )
        
        # Store data in parallel lists
        queries = []
        responses = []
        scores = []
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            ...
            for batch_idx, batch in enumerate(self.eval_dataloader):
                ...
                with torch.no_grad():
                    ...
                    # Decode and process
                    batch_queries = processing_class.batch_decode(query, skip_special_tokens=True)
                    batch_responses = processing_class.batch_decode(postprocessed_response)
                    ...
                    # Store data
                    for q, r, s in zip(batch_queries, batch_responses, score_np):
                        queries.append(str(q) if q is not None else "")
                        responses.append(str(r) if r is not None else "")
            ...
            # # Option 1: Using structured array
            # dtype = [
            #     ('query', 'O'),  # Object type for strings
            #     ('model_response', 'O'),  # Object type for strings
            #     ('score', 'f8')  # float64 for scores
            # ]
            import pandas as pd 
            data_dict = {
                'query': list(queries),
                'model response': list(responses),
                'score': list(scores)
            }
            data = pd.DataFrame(data_dict)
            # Print inspection of numpy array
            # print("\nNumPy array created:")
            ...
            #         print(f"Response: {data[i]['model_response'][:100]}")
            #         print(f"Score: {data[i]['score']}")
                
            if "wandb" in args.report_to and wandb.run is not None:
                # Convert to DataFrame only for wandb logging if needed
                import pandas as pd
